...
public class JoineManager : MonoBehaviour
{
    ...
    void OnLeave(InputAction.CallbackContext context)
    {
        ...
        {
            PlayerDataHolder.Instance.RemoveData(obj,device.deviceId,device.deviceId);
    ...
    void CreatePlayer(InputDevice device){
        ...
        PlayerDataHolder.Instance.SetData(obj.gameObject,playerIndex,device.deviceId);
        DontDestroyOnLoad(obj);
        playerObjects[device.deviceId] = obj.gameObject;

        var inp = obj.GetComponent<PlayerInput>();
        Debug.LogError($"playerIndex={inp.playerIndex}");
        Debug.LogError($"user.id={inp.user.id}");
    }
}
